{{-- 
  ...
  @section('title')
  Used {{$inventory[0]->category}} for Sale | Alberta Crane Service Ltd
  @endsection
  ...
  @endsection
  @section('h1-text')
    <h1 class="text-white uppercase font-bold text-2xl lg:text-4xl">Used {{$inventory[0]->category}} inventory for sale</h1>
  @endsection
  @section('content')
   
  <section id='breadcrumbs' class="p-5">
    <div class="text-start uppercase font-semibold font-xl text-cyan-800"><a href="{{ route('home') }}">Home</a> > <a href="{{ route('inventory') }}">Inventory</a> > <a href="{{ route('category', ['slug' => Str::kebab($inventory[0]->category)]) }}">{{$inventory[0]->category}}</a></div>
  </section>
      ...
      <div class="grid grid-flow-rows grid-cols-1 lg:grid-cols-3 grid-rows-max gap-5">
        ...
        <div class="flex shadow p-2 rounded-xl bg-zinc-50 gap-5 @if ($loop->iteration > 6) fadeIn  @endif" data-category="{{ $crane->category }}">
          ...
          <div class="flex-rows w-full align-center justify-center relative">
            <div class="w-full text-center uppercase text-cyan-800 font-semibold text-md md:text-xl">{{$crane->year}} {{$crane->subject}}</div>
